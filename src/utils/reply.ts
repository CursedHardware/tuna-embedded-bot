...
import type { InlineKeyboardButton, InputMediaPhoto, Update } from 'telegraf/typings/core/types/typegram'
...
interface ReplyOptions {
  ...
  photos?(): Generator<InputMediaPhoto> | InputMediaPhoto[]
...
export async function reply(ctx: Context<Update>, options: ReplyOptions) {
  ...
  const photos: InputMediaPhoto[] = Array.from(options.photos?.() ?? [])
  ...
  if (ctx.chat?.type === 'private' && isPDF(dsURL)) {
    ...
    if (pdfSource) {
      const source = await getPDFPage(pdfSource)
      if (source) photos.unshift({ type: 'photo', media: { source } })
    }
    if (photos.length) {
    ...
    await ctx.replyWithPhoto(photos[0].media, { ...extra, caption })
  } else {
